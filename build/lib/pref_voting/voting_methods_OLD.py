...
def plurality(profile, curr_cands = None):
    ...
    cands_to_ignore = np.array([c for c in profile.candidates if c not in curr_cands]) if curr_cands is not None else np.empty(0)
    plurality_scores = {c: _num_rank_first(rs, rcounts, cands_to_ignore, c) for c in profile.candidates 
...
def hare(profile, curr_cands = None):
...
@vm_name("Minimax")
def minimax(profile, curr_cands = None,  score_method="margins"):
    ...
    scores = {c: max([score_functions[score_method](_c,c) for _c in curr_cands if _c != c]) 
              for c in curr_cands}
    min_score = min(scores.values())
    return sorted([c for c in curr_cands if scores[c] == min_score])
...
@vm_name("Daunou")
def daunou(profile):
    """Implementation of Daunou's voting method as described in the paper: 
...
def stable_voting(profile, curr_cands = None): 
    return stable_voting_(profile, curr_cands = curr_cands, mem_sv_winners = {})[0]
